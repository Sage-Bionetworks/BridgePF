...
public class DynamoUserConsentDaoTest {
    ...
    @Before
    public void before() {
        userConsentDao.deleteAllConsents(HEALTH_CODE, SUBPOP_GUID);
        for (int i=1; i < 6; i++) {
            userConsentDao.deleteAllConsents(HEALTH_CODE+i, SUBPOP_GUID);
        }
    }
    ...
    public void canConsentAndWithdrawFromStudy() {
        ...
        // Withdraw
        userConsentDao.withdrawConsent(HEALTH_CODE, SUBPOP_GUID, DateTime.now().getMillis());
        verifyActiveConsentAbsent();
    ...
    public void canCountStudyParticipants() throws Exception {
        // What this test is point out is that this method is no longer on the consent dao.
        StudyIdentifier studyId = new StudyIdentifierImpl("123h"); 
        ...
        long count = userConsentDao.getNumberOfParticipants(studyId);
        assertEquals("Correct number of participants", 5, count);
        
        userConsentDao.withdrawConsent(HEALTH_CODE+"5", SUBPOP_GUID, DateTime.now().getMillis());
        count = userConsentDao.getNumberOfParticipants(studyId);
    ...
    public void canGetAllConsentsInHistory() throws Exception {
        ...
        List<UserConsent> consents = userConsentDao.getUserConsentHistory(HEALTH_CODE, SUBPOP_GUID);
    ...
    public void singleUserCanSignMultipleConsents() {
        ...
        UserConsent activeConsent = userConsentDao.getActiveUserConsent(HEALTH_CODE, SUBPOP_GUID);
    ...
    public void getConsentBySignedOnDate() throws Exception {
        ...
        UserConsent consent = userConsentDao.getUserConsent(HEALTH_CODE, SUBPOP_GUID, signedOn);
        ...
        userConsentDao.withdrawConsent(HEALTH_CODE, SUBPOP_GUID, withdrewOn);
        
        consent = userConsentDao.getUserConsent(HEALTH_CODE, SUBPOP_GUID, signedOn);
        assertEquals(signedOn, consent.getSignedOn());
        assertEquals(SUBPOP_GUID, consent.getSubpopulationGuid());
    ...
    private void verifyActiveConsentExists() {
        UserConsent active = userConsentDao.getActiveUserConsent(HEALTH_CODE, SUBPOP_GUID);
        assertNotNull(active);
        assertTrue(userConsentDao.hasConsented(HEALTH_CODE, SUBPOP_GUID));
    ...
    private void verifyActiveConsentAbsent() {
        assertNull(userConsentDao.getActiveUserConsent(HEALTH_CODE, SUBPOP_GUID));
        assertFalse(userConsentDao.hasConsented(HEALTH_CODE, SUBPOP_GUID));
    }
    ...
    private void verifyConsentAbsentAt(long signedOn) {
        try {
            userConsentDao.getUserConsent(HEALTH_CODE, SUBPOP_GUID, signedOn);
