...
public class AuthenticationServiceImpl implements AuthenticationService {
	...
	public UserSession signIn(Study study, SignIn signIn)
			throws ConsentRequiredException, BridgeNotFoundException,
			BridgeServiceException {

		if (signIn == null) {
			throw new BridgeServiceException("SignIn object is required",
					HttpStatus.SC_BAD_REQUEST);
		} else if (StringUtils.isBlank(signIn.getUsername())) {
			throw new BridgeServiceException("Username/email must not be null",
					HttpStatus.SC_BAD_REQUEST);
		} else if (StringUtils.isBlank(signIn.getPassword())) {
			throw new BridgeServiceException("Password must not be null",
					HttpStatus.SC_BAD_REQUEST);
		} else if (study == null) {
			throw new BridgeServiceException("Study is required",
					HttpStatus.SC_BAD_REQUEST);
		}
	...
	public void signUp(SignUp signUp) throws BridgeServiceException {
		if (signUp == null) {
			throw new BridgeServiceException("SignUp object is required",
					HttpStatus.SC_BAD_REQUEST);
			...
			throw new BridgeServiceException("Password is required",
					HttpStatus.SC_BAD_REQUEST);
		...
		try {
			...
			Application application = StormpathFactory
					.createStormpathApplication(stormpathClient);
			AccountStore store = application.getDefaultAccountStore();
			Directory directory = stormpathClient.getResource(store.getHref(),
					Directory.class);
	...
	public UserSession verifyEmail(Study study, EmailVerification verification)
			throws BridgeServiceException, ConsentRequiredException {
		if (verification == null) {
			throw new BridgeServiceException("Verification object is required",
					HttpStatus.SC_BAD_REQUEST);
		...
		UserSession session = null;
		try {
			Account account = stormpathClient.getCurrentTenant()
					.verifyAccountEmail(verification.getSptoken());
	...
	public void requestResetPassword(Email email) throws BridgeServiceException {
		if (email == null) {
			throw new BridgeServiceException("Email object is required",
					HttpStatus.SC_BAD_REQUEST);
		}
	...
	public void resetPassword(PasswordReset passwordReset)
		...
		if (passwordReset == null) {
			throw new BridgeServiceException(
					"Password reset object is required",
					HttpStatus.SC_BAD_REQUEST);
		...
		try {
			...
			Account account = application
					.verifyPasswordResetToken(passwordReset.getSptoken());
			account.setPassword(passwordReset.getPassword());
			account.save();
		} catch (ResourceException e) {
			throw new BridgeServiceException(e.getDeveloperMessage(),
					HttpStatus.SC_BAD_REQUEST);
		}
	}

	@Override
	public void consentToResearch(String sessionToken)
			throws BridgeServiceException {
		try {
			...
			Account account = stormpathClient.getResource(
					session.getStormpathHref(), Account.class);
			String key = session.getStudyKey()
					+ BridgeConstants.CUSTOM_DATA_CONSENT_SUFFIX;
			CustomData data = account.getCustomData();
			data.put(key, "true");

			key = session.getStudyKey()
					+ BridgeConstants.CUSTOM_DATA_HEALTH_CODE_SUFFIX;
	...
	private UserSession createSessionFromAccount(Study study, Account account) {
		...
		CustomData data = account.getCustomData();
		String consentKey = study.getKey()
				+ BridgeConstants.CUSTOM_DATA_CONSENT_SUFFIX;
		session.setConsent("true".equals(data.get(consentKey)));
		final String hdcKey = study.getKey()
				+ BridgeConstants.CUSTOM_DATA_HEALTH_CODE_SUFFIX;
