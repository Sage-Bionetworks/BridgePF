...
public class ABTestScheduleStrategy implements ScheduleStrategy {
    ...
    public Schedule getScheduleForUser(Study study, SchedulePlan plan, User user) {
        ...
        int i = 0;
        // Stable random percentage generated by feeding stable but unique seed to Random, based 
        // on this strategy's seed + a stable user seed. Each run this will generate the same number.
        long seed = plan.getGuid().hashCode() + user.getHealthCode().hashCode();
        int perc = new Random(seed).nextInt(100) + 1;
        while (perc > 0) {
            group = groups.get(i++);
